...
WHERE P2.CODPRO = P.CODPRO AND P.CODPUE = C.CODPUE AND C.CODCLI = F.CODCLI AND F.CODFAC = LF.CODFAC AND LF.CODART = A.CODART AND (P.NOMBRE) LIKE ('%a');
...
FROM CLIENTES c, FACTURAS f ,LINEAS_FAC lf ,ARTICULOS a 
WHERE C.CODCLI =F.CODCLI AND F.CODFAC = LF.CODFAC AND LF.CODART = A.CODART AND C.NOMBRE LIKE ('MARIA MERCEDES'); ODCLI = F.CODCLI AND F.CODFAC = LF.CODFAC AND LF.CODART = A.CODART AND UPPER(P.NOMBRE) LIKE ('%A');