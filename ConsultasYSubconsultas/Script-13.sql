...
SELECT CODFAC,FECHA FROM FACTURAS
WHERE ((CODCLI>=50) AND (CODCLI<=100)) AND (EXTRACT (YEAR FROM FECHA) = (EXTRACT(YEAR FROM SYSDATE) -1);
...
SELECT CODFAC,FECHA FROM FACTURAS
WHERE FECHA=TO_DATE('06/2004','MM/YYYY');
...
SELECT CODFAC,FECHA FROM FACTURAS
WHERE FECHA=TO_DATE('06/2004','MM/YYYY') AND (CODCLI BETWEEN 100 AND 250);
...
SELECT CODART,(PRECIO-(PRECIO*0.10)) AS PRECIO_PROMO,PRECIO,DESCRIP AS NOMBRE_ART FROM ARTICULOS WHERE (PRECIO > 6000) AND ((PRECIO*STOCK) > 60000) ;
